...
class IteratedPrisonersDilemmaEnv(MultiAgentEnv):
    ...
    def __init__(self, config=None):
        ...
        single_action_space = spaces.Discrete(2)  # 0=Cooperate, 1=Defect
        
        # Multi-agent spaces must be dictionaries
...
def train_ipd(num_iterations: int = 100, checkpoint_freq: int = 10):
    ...
    config = (
        ...
        .env_runners(
            num_env_runners=8,  # Distribute across cluster
    ...
    for iteration in range(num_iterations):
        ...
        # Extract metrics from the new API
        # The structure is different - episode metrics are in env_runners
        env_runner_results = result.get('env_runners', {})
        
        # Get per-policy rewards if available
        if 'module_episode_returns_mean' in env_runner_results:
            episode_reward_mean_0 = env_runner_results['module_episode_returns_mean'].get('policy_0', 0)
            episode_reward_mean_1 = env_runner_results['module_episode_returns_mean'].get('policy_1', 0)
        else:
            # Fallback: use overall episode return
            episode_reward_mean_0 = env_runner_results.get('episode_return_mean', 0)
            episode_reward_mean_1 = env_runner_results.get('episode_return_mean', 0)
        
        episode_len_mean = env_runner_results.get('episode_len_mean', 100)
        ...
        if iteration % 5 == 0:
            ...
            timesteps = env_runner_results.get('num_env_steps_sampled_lifetime', 0)
